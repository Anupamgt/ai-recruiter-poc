...
def main():
    parser = argparse.ArgumentParser(description="Run dev evaluation pipeline.")
    ...
    # Normalize extracted items into (candidate_dict, raw_score)
    extracted = []
    ...
    # Ensure exactly 100 candidates if fewer were returned
    ...
    # Normalize scores to [0, 100] or maintain relative values
    ...
    # Sort strictly by non-increasing score, then ascending candidate_id for ties
    normalized_items.sort(key=lambda x: (-x[0], x[1]))

    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"Generating reasoning (mode='online') and writing to {out_path}...")
    with open(out_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["candidate_id", "rank", "score", "reasoning"])
        for rank, (score, cid, cand) in enumerate(normalized_items, start=1):
            reasoning = generate_reasoning(cand, mode="online")
